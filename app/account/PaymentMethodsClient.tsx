		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				...
				<button
					...
					className="inline-flex items-center gap-2 rounded-lg bg-brand-accent px-4 py-2.5 text-sm font-medium text-white hover:bg-orange-600 transition-colors"
				>
			...
			<div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
				<h3 className="text-base font-semibold text-slate-900 mb-4">JazzCash</h3>
				{paymentMethods.jazzcash.accountNumber ? (
					<div className="border border-slate-200 rounded-lg p-4 bg-white flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								<div className="font-semibold text-sm text-slate-900">JazzCash</div>
								<div className="text-sm text-slate-600 mt-0.5">{paymentMethods.jazzcash.accountName} • {paymentMethods.jazzcash.accountNumber}</div>
							</div>
						</div>
						<div className="flex items-center gap-2">
							<button
								...
								className="p-2 text-slate-600 hover:text-brand-accent hover:bg-orange-50 rounded-lg transition-colors"
								title="Edit"
							>
					...
					<div className="text-center py-6 text-sm text-slate-500">No JazzCash payment method added</div>
				)}
			</div>

			{/* EasyPaisa Section */}
			<div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
				<h3 className="text-base font-semibold text-slate-900 mb-4">EasyPaisa</h3>
				{paymentMethods.easypaisa.accountNumber ? (
					<div className="border border-slate-200 rounded-lg p-4 bg-white flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								<div className="font-semibold text-sm text-slate-900">EasyPaisa</div>
								<div className="text-sm text-slate-600 mt-0.5">{paymentMethods.easypaisa.accountName} • {paymentMethods.easypaisa.accountNumber}</div>
							</div>
						</div>
						<div className="flex items-center gap-2">
							<button
					...
					<div className="text-center py-6 text-sm text-slate-500">No EasyPaisa payment method added</div>
				)}
			</div>

			{/* Other Banks Section */}
			<div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
				<h3 className="text-base font-semibold text-slate-900 mb-4">Other Banks</h3>
				{paymentMethods.other.length > 0 ? (
					<div className="space-y-3">
						{paymentMethods.other.map((bank, index) => (
							<div key={index} className="border border-slate-200 rounded-lg p-4 bg-white flex items-center justify-between">
								<div className="flex items-center gap-3">
									<img 
										src="/bank.png" 
										alt={bank.bankName} 
									...
									/>
									<div>
										<div className="font-semibold text-sm text-slate-900">{bank.bankName}</div>
										<div className="text-sm text-slate-600 mt-0.5">{bank.accountName} • {bank.accountNumber}</div>
									</div>
								</div>
								<div className="flex items-center gap-2">
									<button
					...
					<div className="text-center py-6 text-sm text-slate-500">No other bank payment methods added</div>
				)}
			</div>

			{/* Add Payment Method Dialog */}
			{showAddPaymentDialog && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowAddPaymentDialog(false)}>
					<div 
							...
							{!selectedBankType ? (
								<>
									...
									<div className="space-y-3">
										<button
											...
											className="w-full p-4 border-2 border-gray-300 rounded-md hover:border-brand-accent hover:bg-brand-accent/5 transition-all flex items-center justify-between"
										>
											<div className="flex items-center gap-3">
												<img src="/jazzcash.png" alt="JazzCash" className="h-8 w-auto object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />
										...
										<button
											...
											className="w-full p-4 border-2 border-gray-300 rounded-md hover:border-brand-accent hover:bg-brand-accent/5 transition-all flex items-center gap-3"
										>
								...
								<>
									...
									{selectedBankType === 'OTHER' && showBankList ? (
										<div className="space-y-3">
											<h4 className="text-sm font-medium mb-2">Select Bank</h4>
											{['HBL', 'UBL', 'Meezan Bank', 'Allied Bank', 'Bank Alfalah', 'MCB Bank', 'Standard Chartered', 'Faysal Bank', 'Askari Bank', 'Bank of Punjab'].map((bankName) => (
												<button
													...
													className="w-full p-3 border-2 border-gray-300 rounded-md hover:border-brand-accent hover:bg-brand-accent/5 transition-all text-left flex items-center gap-3"
												>
													<img 
														src="/bank.png" 
														alt={bankName} 
										...
										<div className="space-y-4">
											...
											<div className="flex gap-3 mt-6">
												<button 
													...
													className="flex-1 h-[50px] rounded-lg border border-slate-300 px-6 py-3 text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 transition-colors disabled:opacity-50"
													disabled={submitting}
												>
													Cancel
												</button>
												<button 
													...
													className="flex-1 h-[50px] rounded-lg bg-brand-accent px-6 py-3 text-sm font-medium text-white hover:bg-orange-600 transition-colors disabled:opacity-50"
													disabled={submitting}
												>
