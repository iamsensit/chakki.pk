...
function CartPanel() {
	...
	return (
		<div className="rounded-md border">
				...
				{items.map((i, idx) => (
					<div key={idx} className="p-3 flex items-center gap-3 text-sm">
						...
						<div className="flex items-center gap-2">
							<button aria-label="Decrease" className="h-6 w-6 rounded border" onClick={() => updateQuantity(i.productId, i.variantId ?? null, Math.max(0, i.quantity - 1))}>-</button>
							<div className="w-6 text-center">{i.quantity}</div>
							<button aria-label="Increase" className="h-6 w-6 rounded border" onClick={() => updateQuantity(i.productId, i.variantId ?? null, i.quantity + 1)}>+</button>
						</div>
						<div className="text-sm font-semibold min-w-[72px] text-right">Rs. {i.unitPrice * i.quantity}</div>
					</div>
				))}
			</div>
			...
			<div className="p-3">
				<a href="/checkout" className="block w-full text-center rounded-md bg-brand-accent px-3 py-1.5 text-white text-sm">Go to checkout</a>
			</div>
		</div>
	)
}
		...
		<div className="grid gap-6 lg:grid-cols-12">
			<aside className="lg:col-span-3">
				<div className="rounded-md border overflow-hidden bg-white">
					<button onClick={() => setTab('profile')} className={`w-full flex items-center gap-2 px-4 py-3 text-sm border-b transition-colors ${tab === 'profile' ? 'bg-brand text-white' : 'hover:bg-gray-50'}`}>
						<User className="h-4 w-4" /> Profile & Security
					</button>
					<button onClick={() => setTab('orders')} className={`w-full flex items-center gap-2 px-4 py-3 text-sm border-b transition-colors ${tab === 'orders' ? 'bg-brand text-white' : 'hover:bg-gray-50'}`}>
						<ClipboardList className="h-4 w-4" /> Orders
					</button>
					<button onClick={() => setTab('cart')} className={`w-full flex items-center gap-2 px-4 py-3 text-sm transition-colors ${tab === 'cart' ? 'bg-brand text-white' : 'hover:bg-gray-50'}`}>
						<ShoppingCart className="h-4 w-4" /> Server Cart
					</button>
				</div>
			</aside>
